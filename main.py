...
def train_one_epoch_shared_epsilon(
    model,
    dataloader,
    diffusion,
    optimizer,
    ...
    for step, (video_frames, optical_flow, labels) in data_iter:
        # optical_flow shape: (b, 2) - 2 values for differences between frames
        ...
        video_frames = 2 * video_frames.to(args.device) - 1  # (b, 3, c, h, w)
        optical_flow = optical_flow.to(args.device)  # (b, 2)
        labels = labels.to(args.device) if args.class_cond else None
        
        # Sample the same timestep for all frames in a sequence
        t = torch.randint(diffusion.timesteps, (batch_size,), dtype=torch.int64).to(args.device)
        
        # Reshape video frames for processing
        frames_flat = video_frames.view(-1, channels, height, width)  # (b*3, c, h, w)
        t_flat = t.repeat_interleave(num_frames)  # (b*3)
        
        eps = torch.randn(batch_size, channels, height, width, device=args.device)
        eps_expanded = eps.unsqueeze(1).expand(-1, num_frames, -1, -1, -1)  # (b, 3, c, h, w)
        eps_flat = eps_expanded.reshape(-1, channels, height, width)  # (b*3, c, h, w)
        
        # Forward diffusion process with shared noise
        xt_flat, _ = diffusion.sample_from_forward_process(frames_flat, t_flat, eps=eps_flat)
        
        # Model prediction
        pred_eps_flat = model(xt_flat, t_flat, y=labels.repeat_interleave(num_frames) if labels is not None else None)
        ...
        # Regularization loss: minimize differences between predicted epsilons of consecutive frames
        # Compute differences between frames 0-1 and 1-2
        eps_diff_0_1 = ((pred_eps[:, 0] - pred_eps[:, 1]) ** 2).mean(dim=[1, 2, 3])  # (b)
        eps_diff_1_2 = ((pred_eps[:, 1] - pred_eps[:, 2]) ** 2).mean(dim=[1, 2, 3])  # (b)
        
        # Inverse weighting using optical flow values
        # Higher optical flow value = more different frames = less regularization
        flow_weight_0_1 = 1.0 / (1.0 + optical_flow[:, 0])  # (b)
        flow_weight_1_2 = 1.0 / (1.0 + optical_flow[:, 1])  # (b)
        
        # Apply weighting to the epsilon differences
        ...
        # Final loss: original MSE + weighted regularization
        reg_weight = getattr(args, "reg_weight", 0.1)  # Default weight if not specified
