...
def get_dataset(name, data_dir, metadata, num_training_data=None, preload_hanco=False):
    """
    ...
    if name == "mnist":
        ...
        if num_training_data is not None:
            train_set = torch.utils.data.Subset(train_set, range(num_training_data))
    elif name == "mnist_m":
        transform_train = transforms.Compose(
            [
                transforms.RandomResizedCrop(
        ...
        if num_training_data is not None:
            train_set.samples = train_set.samples[:num_training_data]
            if hasattr(train_set, 'imgs'):
                train_set.imgs = train_set.imgs[:num_training_data]
    elif name == "cifar10":
        transform_train = transforms.Compose(
            [
        ...
        all_indexes = np.concatenate((splits["trnid"][0], splits["valid"][0]), axis=0)
        if num_training_data is not None:
            all_indexes = all_indexes[:num_training_data]
