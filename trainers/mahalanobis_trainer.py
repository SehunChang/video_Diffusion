...
class MahalanobisTrainer(BaseTrainer):
    ...
    def __init__(self, model, diffusion, args, **kwargs):
        ...
        self.use_var_weighting = kwargs.get('use_var_weighting', True)
        self.var_eps           = kwargs.get('var_eps', 1e-3)   # δ stabiliser
        ...
        print(f"use_var_weighting: {self.use_var_weighting}")
        print(f"var_eps: {self.var_eps}")
    ...
    def train_one_epoch(self, dataloader, optimizer, logger, lrs):
        ...
        for step, (video_frames, optical_flow) in data_iter:
            ...
            reg_loss = wsm_loss
            # Final loss combining MSE, WSM, and regularization
